...
private:
    ...
    double posdet(MatrixXd R, int n);
...
void universe::set_search()
{
    ...
    if(strcmp(fitfunc,"f_vlmop2")==0) // fitness function name, as it will be given on command line
    {
        ...
        absmax[1]=1.0;
        absmin[1]=0.0;
        MAX_ITERS = 100; // edit for a different number of iterations (evaluations)
    }
    ...
    else if(strcmp(fitfunc,"f_oka1")==0)
    {
        ...
        xmin = (double *)malloc((dim)*sizeof(double));
        xmax = (double *)malloc((dim)*sizeof(double));
        xmin[0]=6*sin(PI/12.0);
        xmax[0]=xmin[1]+2*PI*cos(PI/12.0);
        xmin[1]=-2*PI*sin(PI/12.0);
        xmax[1]=6*cos(PI/12.0);
        gideal[0]=0.0;
        gideal[1]=0.0;
        gwv[0]=0.9;
        gwv[1]=0.1;
...
double universe::posdet(MatrixXd R, int n)
{
...
{    //DON't
    ...
    *ax = (double **)malloc((n)*sizeof(double *));
    for(int i=0;i<n;i++)
        (*ax)[i]=(double *)malloc((dim)*sizeof(double));
...
void universe::iterate_ParEGO()
{
    ...
    double **param;
    param = (double **)malloc((pdim+1)*sizeof(double *));
    for(int i=0;i<=pdim;i++)
        param[i]=(double *)malloc((pdim)*sizeof(double));
    
    
    
    //double bestparam[pdim+2][pdim+1];
    double **bestparam;
    bestparam = (double **)malloc((pdim+1)*sizeof(double *));
    for(int i=0;i<=pdim;i++)
        bestparam[i]=(double *)malloc((pdim)*sizeof(double));
    
    double *p_last;
    double *theta_last;
    p_last=(double *)malloc((dim)*sizeof(double));
    theta_last=(double *)malloc((dim)*sizeof(double));
    double sigma_last;
    double mu_last;
    
    
    
    double tryx[dim+1][dim];
    // double **tryx;
    ...
    double **popx;
    popx = (double **)malloc((gapopsize)*sizeof(double *));  // GA population for searching over modelled landscape
    for(int i=0;i<gapopsize;i++)
        popx[i]=(double *)malloc((dim)*sizeof(double));
...
void universe::init_ParEGO()
{
    ...
    ff = (double **)malloc((MAX_ITERS+1)*sizeof(double *));
    for(int i=0;i<MAX_ITERS+1;i++)
        ff[i]=(double *)malloc((dim)*sizeof(double));
...
double universe::predict_y(double **ax, MatrixXd InvR, VectorXd y, double mu_hat, double *theta, double *p, int n, int dim)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        //fprintf(stdout,"r[%di]=%lf ax[n+1][1]=%lf\n",i, r[i], ax[n+1][1]);
    }
    // fprintf(stdout,"\n");
    ...
    y_hat = mu_hat + (double)((r.transpose()*InvR)*(y-(one*mu_hat)));
    
    // fprintf(stderr,"y_hat=%lf mu_hat=%lf\n",y_hat, mu_hat);
    /*
...
double universe::s2(double **ax, double *theta, double *p, double sigma, int dim, int n, MatrixXd InvR)
{
    ...
    double f1 = (double)(r.transpose()*InvR*r);
    double f2 = (double)(one.transpose()*InvR*r);
    double f4  = 1.0 - f2;
    double f3 = (double)(one.transpose()*InvR*one);
...
void mysort(int *idx, double *val, int num)
{
    ...
    qsort(&(ss[0]),num,sizeof(SOR), pcomp);
