...
int GeneticAlgorithm::thread_run(SearchSpace* space, DACE* model, int iter)
{
    //        //fprintf(stderr,"GA iter %d ", i);
    //        //fprintf(stderr,"popx ");
...
//   printf("popy ");
    ...
    for (int i=1; i<=8; i++)
    {
        std::async(std::launch::async, &GeneticAlgorithm::thread_run, this, space, model, iter);
        //threads[i].join();
    }
