...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr, "%s\n", function.c_str());
    string filename = function+ "-obj-it7-what-GA" + to_string(U.MAX_ITERS) ;
...
void universe::iterate_ParEGO()
{
    ...
    for(int i=1;i<=iter;i++)
    {
        ...
        //fprintf(stderr,"%lg ", space->fMeasuredFit[i]);
    ...
    for(int d=1;d<=space->fSearchSpaceDim;d++)
    {
        ...
        //fprintf(stderr, "%lg ", space->fXVectors[iter+1][d]);
    }
    //fprintf(stderr, "\n");
