...
int GeneticAlgorithm::tourn_select(double *xsel, double **ax, double *ay, int iter, int t_size, SearchSpace* space)
{
    ...
    int bestidx = 1;
    Utilities::cwr(&idx, t_size, iter);                    // fprintf(stdout,"cwr\n");
...
//   printf("popy ");
    ...
    //	fprintf(stdout,"ay = %.5lf ; %.5lf\n", ay[ind[i]], ay[i]);
    
    //fprintf(stdout,"Begin_MUTATION");
    
    for (int i=1; i<=5; i++)
    {
        //	  fprintf(stdout,"parent = %.5lf\n", ay[ind[i]]);
        parA=ind[i];
        for(int d=1;d<=space->fSearchSpaceDim;d++)
            popx[i][d] = space->fXVectors[parA][d];
        mutate(popx[i], space);                                // fprintf(stdout,stdout, "mutate\n");
        //        for(int j=1; j < space->fSearchSpaceDim+1 ;j++)
        //            fprintf(stdout,"%lg ", popx[i][j]);
        popy[i] = model->wrap_ei(popx[i], iter);                   // fprintf(stdout,stdout, "evaluate\n");
        
    }
    
    //fprintf(stdout,"End_MUTATION");
    
    
    double p_cross=0.2;
    for (int i=1; i<=10000; i++)
    {
        //        //fprintf(stdout,"GA iter %d ", i);
        //        //fprintf(stdout,"popx ");
        //        for(int i = 1; i < gapopsize + 1; i++)
        //            for (int j = 1; j < space->fSearchSpaceDim+1; j++)
        //            {
        ...
        if(rn < p_cross)
        {
            ...
            do
            {
                //                fprintf(stdout,"TOURN");
                //                for (int i=1; i<=gapopsize; i++)
                //                {
                //                    fprintf(stdout,"%lg ", popy[i]);
                //                }
                //                fprintf(stdout,"TOURN_END\n");
            ...
            parA=tourn_select(mutx, popx, popy, gapopsize, 2, space);//  fprintf(stdout, "parent selected\n");
        mutate(mutx, space);                             // fprintf(stdout, "mutate\n");
        muty = model->wrap_ei(mutx, iter);                      //  fprintf(stdout, "evaluate\n");
        if(muty<popy[parA])
        {
            for(int d=1;d<=space->fSearchSpaceDim;d++)
            {
