...
void DACE::build_R(double **ax, MatrixXd& R)
{
    ...
    for(int i=0;i<fCorrelationSize;i++)
    {
        for(int j=0;j<fCorrelationSize;j++)
        {
            ...
            R(i,j)=correlation(ax[i+1],ax[j+1], gtheta, gp, fCorrelationSize);
            if(Debug)
                fprintf(stderr,"%.5lf\n", R(i,j));
        }
        if(Debug)
...
double DACE::s2(double **ax, int iter)
{
    if(Debug)
        printf("sigma: %f\n", gsigma);
    double s2;
    ...
    VectorXd r(fCorrelationSize);
    for(int i=0;i<fCorrelationSize;i++)
    {
        //fprintf(stderr,"theta=%.5lf p=%.5lf ax[n+1]=%.5lf, ax[%d+1]=%.5lf\n", gtheta[i+1],gp[i+1],ax[fCorrelationSize+1][i+1],i, ax[i+1][i]);
        r(i) = correlation(ax[fCorrelationSize+1],ax[i+1],gtheta,gp,daceSpace->fSearchSpaceDim);
    ...
    {
        *res1 = r.transpose()*pInvR*r;
        *res2 = one.transpose()*pInvR*r;
...
double DACE::mu_hat(VectorXd& y, int iter)
{
    VectorXd one(fCorrelationSize);
    for(int i=0;i<fCorrelationSize;i++)
        one(i)=1;
    ...
    float* denominator = (float*)malloc(1*1*sizeof(float));
    ...
    if (iter > 10)
    {
        ...
        matrixMul(1, fCorrelationSize, fCorrelationSize, fCorrelationSize, 
                  1, fCorrelationSize,
                 one_transpose, pInvRGPU, interm1);
        matrixMul(1, fCorrelationSize, fCorrelationSize, 1, 1, 1,
                  interm1, y_gpu, numerator);
        matrixMul(1, fCorrelationSize, fCorrelationSize, 1, 1, 1,
                  interm1, one_transpose, denominator);
    }
    else 
    {
...
double DACE::sigma_squared_hat(VectorXd& y, int iter)
{
    double denominator;
...
double DACE::likelihood(double *param, double** pax, double* pay)
{
    ...
    MatrixXd R = MatrixXd::Zero(fCorrelationSize,fCorrelationSize);
    build_R(pax, R);
    ...
    double detR = posdet(R,fCorrelationSize);
    
    //  pr_sq_mat(R, fCorrelationSize);
    
    //  fprintf(stderr,"determinant=%lg\n", detR);
    ...
    // fprintf(stderr,"exponent = %lg", exponent);
    // fprintf(stderr, "likelihood = %lg\n", lik);
...
void DACE::buildDACE(bool change, int iter)
{
    ...
    if(change=1)
    {
        ...
        for(int i=0;i<30;i++)
        {
            ...
            MatrixXd R = MatrixXd::Zero(fCorrelationSize, fCorrelationSize);
            build_R(*pax, R);
            //  fprintf(stderr,out,"NEW STARTING R Matrix\n");
            // pr_sq_mat(R,iter);
            
            double detR=posdet(R,fCorrelationSize);
            
            //fprintf(stderr,"det = %.5lf\n", R.determinant());
            // pr_sq_mat(R,fCorrelationSize);
            //cout<< "THe MATRIX:"<<R<<"\n";
            pInvR = R.inverse();
...
//    
    ...
    //ME: Carefull with the indexes!!! It starts from 1.
    MatrixXd pgR = MatrixXd::Zero(fCorrelationSize,fCorrelationSize);
    build_R(*pax, pgR);
    posdet(pgR, fCorrelationSize);
