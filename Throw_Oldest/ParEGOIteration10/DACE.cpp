...
//    
    ...
    if (iter > 11*daceSpace->fSearchSpaceDim+24)
    {
        fCorrelationSize = (iter > NMAX)? NMAX : fCorrelationSize;
        //start index, forget old values heuristic
        int start = (iter > NMAX) ? iter-fCorrelationSize : 10;
        *pax = &daceSpace->fXVectors[start];
        *pay = &daceSpace->fMeasuredFit[start];
        //fprintf(stderr, "size: %d\n", start);
    }
    else {
        fCorrelationSize=iter;
        pax = &daceSpace->fXVectors;
        pay = &daceSpace->fMeasuredFit;
    }
    //fprintf(stdout, "\n");
    ...
    pgy = MyVector(fCorrelationSize, *pay);
    /* ***************************** */
