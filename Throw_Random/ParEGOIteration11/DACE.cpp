...
//    
    ...
    if (iter > NMAX)
    {
        ...
        int ind[iter+1];
        //Utilities::mysort(ind, daceSpace->fMeasuredFit, iter);
        int *choose;
        // cwr(k,n) - choose without replacement k items from n
        Utilities::cwr(&choose, NMAX, iter);
        

        for(int i = 1 ;i <= fCorrelationSize; i++)
        {
            //asert for bounds in array
            assert(choose[i] <= iter && 0 < choose[i]);
            int j= choose[i];
            for(int d=1;d <=daceSpace->fSearchSpaceDim;d++)
                daceSpace->fSelectedXVectors[i][d] = daceSpace->fXVectors[j][d];
            daceSpace->fSelectedMeasuredFit[i] = daceSpace->fMeasuredFit[j];
        }
